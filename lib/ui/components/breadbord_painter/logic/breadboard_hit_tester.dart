...
class BreadboardHitTester {
  static BreadboardHoverState? hitTest(
    ...
    // Left Power Rail
    final leftRailResult = _testPowerRail(
      localPosition,
      config,
      rail,
      isRight: false,
    ...
    // Right Power Rail
    final rightRailResult = _testPowerRail(
      localPosition,
      config,
      rail,
      isRight: true,
  ...
  static BreadboardHoverState? _testPowerRail(
    ...
    final double baseOffsetX = isRight
    ...
    if (localY < config.firstRowY - hitRadius ||
        localY > config.lastRowY + hitRadius) {
      return null;
    }

    if ((localX - rail.dot1Offset).abs() < hitRadius) {
      return BreadboardHoverState(
        channel: BreadboardChannel.plus,
        isRightSide: isRight,
      );
    }
    if ((localX - rail.dot2Offset).abs() < hitRadius) {
      return BreadboardHoverState(
        channel: BreadboardChannel.minus,
        isRightSide: isRight,
      );
    }
    return null;
