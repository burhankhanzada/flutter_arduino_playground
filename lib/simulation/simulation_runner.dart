...
class SimulationRunner {
  ...
  SimulationRunner({required this.canvasController});
  ...
  LEDPainter? _getLedPainterForPin(String pin) {
    final arduinoNode = canvasController.nodes.firstWhere((n) => n.componentModel.name == 'Arduino Uno');
    ...
    final nextNodeKey = isStartAtArduino ? wire1.end.nodeKey : wire1.start.nodeKey;
    final nextNode = canvasController.nodes.firstWhere((n) => n.key == nextNodeKey);
    
    if (nextNode.componentModel.name == 'LED') return nextNode.componentModel.painter as LEDPainter;
    
    if (nextNode.componentModel.name == 'Resistor') {
       final resistorPortConnected = isStartAtArduino ? wire1.end.portId : wire1.start.portId;
       ...
       final ledNodeKey = isStartAtResistor ? wire2.end.nodeKey : wire2.start.nodeKey;
       final ledNode = canvasController.nodes.firstWhere((n) => n.key == ledNodeKey);
       
       if (ledNode.componentModel.name == 'LED') return ledNode.componentModel.painter as LEDPainter;
    }
    return null;
  }
  ...
  Future<void> _runLoop(String code, void Function() onStop) async {
    ...
    final statements = cleanBody.split(';').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
    ...
    while (isSimulating) {
      ...
      for (final statement in statements) {
        ...
        if (statement.startsWith('digitalWrite')) {
          final pinMatch = RegExp(r'digitalWrite\(\s*(\w+)\s*,\s*(HIGH|LOW)\s*\)').firstMatch(statement);
          if (pinMatch != null) {
          ...
          final delayMatch = RegExp(r'delay\(\s*(\d+)\s*\)').firstMatch(statement);
          if (delayMatch != null) {
